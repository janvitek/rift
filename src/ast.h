...
namespace ast {
    ...
    public:
        Fun(): body(nullptr) {}
        Fun(ast::Exp * body): body(dynamic_cast<ast::Seq *>(body)) { }
        ~Fun() {
    ...
    public:
        EvalCall(ast::Exp * arg) { args.push_back(arg); }
        void accept(Visitor * v) override;
    };
    /** Call to length().  */
    class LengthCall : public SpecialCall {
    public:
        LengthCall(ast::Exp * arg) { args.push_back(arg); }
        void accept(Visitor * v) override;
    };
    /** Call to a type().  */
    class TypeCall : public SpecialCall {
    public:
        TypeCall(ast::Exp * arg) { args.push_back(arg); }
        void accept(Visitor * v) override;
    };
    /** Indexed read.  */
    class Index : public Exp {
    public:
        Index(Exp * name): name(name), index(nullptr) { }
        ~Index() {
            delete name;
            delete index;
        }
    ...
    public:
        SimpleAssignment(Var * name): name(name), rhs(nullptr) { }
        ~SimpleAssignment() {
            delete name;
            delete rhs;
        }
    ...
    public:
        IndexAssignment(Index * index): index(index), rhs(nullptr) { }
    ...
    public:
        WhileLoop(Exp * guard): guard(guard), body(nullptr) { }
        ~WhileLoop() override {
            delete guard;
            delete body;
        }
